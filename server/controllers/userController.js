        ...
        db.query(userTeamQueryString, userTeamValues)
          ...
          .catch((err) => {
            ...
            return next({
              ...
              status: 501,
              message: 'Failed to execute query to add user_id and team_id to userTeam table',
            })
          });
      }
    })
...
userController.joinTeam = (req, res, next) => {
  ...
  db.query(teamQueryString, teamValues)
    ...
    if (res.locals.teamAdded === false) return;
  ...
  //For some reason it executes this query even though I return next above.
  db.query(userTeamQueryString, userTeamValues)
    .then((data) => {
      console.log('in join team query 2')
      //if insert successful, send something on res.locals to frontend
